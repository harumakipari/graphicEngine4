    ...
    {// 左ボタンを押している間
        ...
        float currentLength = DirectX::XMVectorGetX(
            DirectX::XMVector3Length(
                XMLoadFloat2(&cursor) -
                XMLoadFloat2(&dragStartMousePos)));

        currentLength = std::max<float>(0.0f, currentLength);
        const float sensitivity = 0.01f; // 感度調整
        float stretch = baseStretchRate + currentLength * sensitivity;
        const float maxStretch = 10.0f;
        stretch = std::clamp(stretch, 1.0f, maxStretch);
        elasticConstants.stretchRate = stretch;
