...
public:
    ...
    virtual void DrawImGuiInspector() override
    {
        ...
        if (ImGui::TreeNode((name_ + " point Light").c_str()))
        {
            ...
            ImGui::SliderFloat("range", &sharedParam->range, 0.0f, +10.0f);
            ImGui::ColorEdit3("Color", &sharedParam->color.x);
            ImGui::SliderFloat("Intensity##", &sharedParam->color.w, 0.0f, 30.0f);
            ImGui::TreePop();
        }
#endif
    }
    ...
    LightManager::PointLight ToRenderLight() const
    {
        ...
        l.position = {
            GetComponentLocation().x,
                       ...
                       1.0f };
        l.color = sharedParam->color;
        l.range = sharedParam->range;
        return l;
    }
