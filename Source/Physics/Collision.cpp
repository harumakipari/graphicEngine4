...
//
//    auto model = meshComponent->model.get();
//	// モデル内の全てのメッシュと交差判定を行う
//	for (const InterleavedGltfModel::Mesh& mesh : model->meshes)
//	{
//		// メッシュのワールド行列を求める
//		const InterleavedGltfModel::Node& node = model->GetNodes().at(mesh.nodeIndex);
