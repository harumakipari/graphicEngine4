...
void GameTimerUiActor::Initialize(const Transform& transform)
{
    ...
    timerFrameImage = std::make_shared<UIImageComponent>(
        "./Data/Textures/ScissorsUI/timer_frame.png",
        "timer_frame"
    );

    timerFrameImage->SetSize({ 400, 190 });
...
void GameTimerUiActor::Update(float elapsedTime)
{
    ...
    timerDigits[0]->SetWorldPosition({ uiPos.x - spacing * 2.0f, uiPos.y });
    timerDigits[1]->SetWorldPosition({ uiPos.x - spacing * 1.0f, uiPos.y });

    timerFrameImage->SetWorldPosition({ uiPos.x, uiPos.y });

    timerDigits[2]->SetWorldPosition({ uiPos.x + spacing * 1.0f, uiPos.y });
    timerDigits[3]->SetWorldPosition({ uiPos.x + spacing * 2.0f, uiPos.y });
}
