...
void ResultScene::Start()
{
    ...
    audioComp->SetSource(L"./Data/Sound/BGM1/result_bgm.wav");
    ...
    // タイトルへ戻るボタンを生成
    {
        ...
        backToTitleButton->SetWorldPosition({ -162, 985 });
        backToTitleButton->SetSize({ 465, 150 });
        backToTitleButton->SetPivot({ 0.0f,0.5f });
        backToTitleButton->SetUseHoverScale(true);
        ...
        backToTitleButton->onClick = [&]()
            {
                ...
                backToTitleButton->SetEnable(false);
    ...
    // リトライボタンを生成
    {
        retryButton = std::make_shared<UIButtonComponent>("./Data/Textures/UI/Result/retry.png", "retryButton");
        retryButton->SetWorldPosition({ -162, 815 });
        retryButton->SetSize({ 465, 150 });
        retryButton->SetUseHoverScale(true);
        retryButton->SetPivot({ 0.0f,0.5f });
        ...
        retryButton->onClick = [this]()
            {
                ...
                const char* types[] = { "0", "1" };
#if 1
                SceneTransitionManager::Instance().RequestTransition("LoadingScene", { std::make_pair("preload", "GameScene"), std::make_pair("type", types[rand() % 2]) });
...
void ResultScene::UpdateConstants(ID3D11DeviceContext* immediateContext, float deltaTime)
{
...
void ResultScene::SetUpActors()
{
    ...
    mainCameraComponent->SetPitch(DirectX::XMConvertToRadians(-60.0f));
    mainCameraComponent->SetYaw(DirectX::XMConvertToRadians(180.0f));
