...
float4 main(float4 position : SV_POSITION, float2 texcoord : TEXCOORD) : SV_TARGET
{
    ...
    //float coc = abs(viewSpaceZ - focusDistance) / dofRange;
    //coc = saturate(coc);
    //coc = pow(coc, 2.0);

    //float blurScale = coc * blurStrength;
    ...
    for (int i = 1; i < 3; i++)
    {
        sampled_color += hdr_color_buffer_texture.Sample(samplerStates[LINEAR_BORDER_BLACK], texcoord + float2(0.0, offset[i] / height)) * weight[i];
        sampled_color += hdr_color_buffer_texture.Sample(samplerStates[LINEAR_BORDER_BLACK], texcoord - float2(0.0, offset[i] / height)) * weight[i];
    }
#else
